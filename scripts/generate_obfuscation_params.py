...
Сознательно ограничено проверенным набором (Jc/Jmin/Jmax/S1/S2/H1-H4) —
S3/S4 и I1-I5 пробовали добавить все сразу, оказалось, что на текущей
версии kernel-модуля это ломает соединение целиком (даже на WiFi, где
раньше было стабильно). Добавлять обратно нужно по одному параметру,
с тестом на отдельном интерфейсе (см. ADVANCED.md), а не пакетом сразу —
иначе невозможно понять, что конкретно не поддерживается.

Ограничения на значения (проверено эмпирически):
